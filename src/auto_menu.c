...
void mainmenu__print_voce(int curr_voce, _Bool stato_bloccoporte,
  ...
  int len_voci[] = {19, 16, 10, 25, 11, 10, 16, 21};
  ...
  // Se la voce corrente è blocco porte o back home, ne stampo lo stato
  if (curr_voce == 3) {
...
void submenu_adv__display_menu(int menuid, int *stato_freccedirezione) {
  ...
  while (1) {
    ...
    if (usrcmd == 1) {
      // Se ho raggiunto il minimo numero di lampeggi vado a 5
      if (*stato_freccedirezione == 2)
	*stato_freccedirezione = 5;
      else
	// Altrimenti lo decremento
	(*stato_freccedirezione)--;
    } else if (usrcmd == 2) { // Se invece ha inserito DOWN
      // Se ho raggiunto il massimo numero di lampeggi torno a 2
      if (*stato_freccedirezione == 5)
	*stato_freccedirezione = 2;
      else
	// Altrimenti lo incremento
	(*stato_freccedirezione)++;
    } else if (usrcmd == 4) {
      // Esco quando viene inserito invio
      return;
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    mainmenu__print_voce(curr_voce, stato_bloccoporte, stato_backhome);
